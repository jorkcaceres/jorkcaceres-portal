...
import { createDiagnostic } from './diagnostic/ui.js?v=1.1.2';
...
const privateRoutes = new Set(['inicio', 'proyectos', 'servicios', 'encuestas', 'admin', 'admin-clientes', 'admin-proyectos', 'admin-servicios', 'admin-pagos', 'admin-encuestas', 'admin-portal', 'diagnosticos', 'admin-diagnosticos']);
...
async function projectsView() {
  ...
  const cards = data.length ? data.map(p => `<article class="card"><div class="card-top"><div><p class="eyebrow">${esc(p.code)}</p><h3>${esc(p.title)}</h3></div><span class="status ${projectStatusClass(p.status)}">${status(p.status)}</span></div><p>${esc(p.service)}</p><div class="item-grid"><span>Fecha<strong>${date(p.project_date)}</strong></span><span>Servicio<strong>${esc(p.service)}</strong></span></div><div class="modal-actions">${btn('Información', `projectInfo('${p.id}')`, 'small secondary')}${btn('Pagos', `projectPayments('${p.id}')`, 'small')}</div></article>`).join('') : '<div class="empty">Aún no tienes proyectos registrados en el portal.</div>';
...
async function projectPayments(id) {
  ...
  const cards = data.map(payment => `<article class="payment-detail-card"><span class="payment-code">${esc(payment.code)}</span><span class="status ${paymentStatusClass(payment.status)}">${paymentStatus(payment.status)}</span><p class="payment-summary">${esc(paymentType(payment.payment_type))} · <strong>${payment.amount ? money(payment.amount) : 'Monto por definir'}</strong>${payment.status === 'confirmado' ? ` <small>· ${date(payment.payment_date)}</small>` : ''}</p>${payment.receipt_path ? btn('Ver comprobante', `openPaymentReceipt('${esc(payment.receipt_path)}')`, 'small secondary') : ''}</article>`).join('');
...
const adminNav = (active) => `<nav class="admin-nav"><a class="${active === 'admin' ? 'active' : ''}" href="#admin">Panorama</a><a class="${active === 'clientes' ? 'active' : ''}" href="#admin-clientes">Clientes</a><a class="${active === 'proyectos' ? 'active' : ''}" href="#admin-proyectos">Proyectos</a><a class="${active === 'servicios' ? 'active' : ''}" href="#admin-servicios">Servicios</a><a class="${active === 'pagos' ? 'active' : ''}" href="#admin-pagos">Pagos</a><a class="${active === 'encuestas' ? 'active' : ''}" href="#admin-encuestas">Encuestas</a><a class="${active === 'diagnosticos' ? 'active' : ''}" href="#admin-diagnosticos">Diagnósticos</a><a class="${active === 'portal' ? 'active' : ''}" href="#admin-portal">Portal</a></nav>`;
...
async function saveDefaultFilters(event) {
  ...
  const allowed = {
    ...
    default_projects_filter: ['todos', 'planificado', 'en_curso', 'pausado', 'finalizado', 'cancelado'],
    default_services_filter: ['todos', 'activo', 'inactivo', 'vencido'],
    default_payments_filter: ['todos', 'pendiente', 'confirmado', 'cancelado'],
  };
...
async function adminProjectsView() {
  ...
  const body = `<div class="admin-module-actions">${btn('Crear proyecto', 'showProjectForm()', 'primary')}<label class="filter-select"><span>Filtrar por estado</span><select aria-label="Filtrar proyectos por estado" onchange="changeProjectFilter(this.value)"><option value="todos" ${filter === 'todos' ? 'selected' : ''}>Mostrar todo</option><option value="planificado" ${filter === 'planificado' ? 'selected' : ''}>Planificados</option><option value="en_curso" ${filter === 'en_curso' ? 'selected' : ''}>En curso</option><option value="pausado" ${filter === 'pausado' ? 'selected' : ''}>Pausados</option><option value="finalizado" ${filter === 'finalizado' ? 'selected' : ''}>Finalizados</option><option value="cancelado" ${filter === 'cancelado' ? 'selected' : ''}>Cancelados</option></select></label></div>${list}`;
...
function projectCard(project) {
  ...
  return `<article class="admin-list-card"><div><p class="eyebrow">${esc(project.code)}</p><h2>${esc(project.title)}</h2><p>${esc(clientName)} · ${esc(project.service)} · ${dates}</p><div class="client-actions">${btn('Modificar proyecto', `showProjectEditForm('${project.id}')`, 'small secondary')}</div></div><span class="status ${projectStatusClass(project.status)}">${status(project.status)}</span></article>`;
}
...
async function adminServicesView() {
  ...
  try {
    ...
    const renewalsRelation = filter === 'vencido'
      ? 'service_renewals!inner(id,renewal_date,status,receipt_path,renewed_at)'
...
async function adminPaymentsView() {
  ...
  const allPayments = [...projectPayments, ...servicePayments]
    .filter(payment => filter === 'todos' || (filter === 'pendiente' ? payment.source === 'project' && payment.status === 'pendiente' : filter === 'cancelado' ? payment.source === 'project' && payment.status === 'cancelado' : (payment.source === 'service' || payment.status === 'confirmado')))
...
function paymentCard(payment) {
  ...
  return `<article class="admin-list-card"><div><p class="eyebrow">${esc(payment.code)}</p><h2>${paymentType(payment.payment_type)}</h2><p>${esc(payment.projects?.title || 'Proyecto no disponible')} · ${esc(clientName)} · ${payment.amount ? money(payment.amount) : 'Sin monto registrado'} · ${payment.status === 'confirmado' ? date(payment.payment_date) : paymentStatus(payment.status)}</p><div class="client-actions">${btn('Modificar pago', `showPaymentEditForm('${payment.id}')`, 'small secondary')}${payment.receipt_path ? btn('Ver comprobante', `openPaymentReceipt('${esc(payment.receipt_path)}')`, 'small secondary') : ''}</div></div><span class="status ${paymentStatusClass(payment.status)}">${paymentStatus(payment.status)}</span></article>`;
}
...
function togglePaymentDetails(status) {
  ...
  if (note) { note.hidden = showPaymentDetails; note.textContent = status === 'pendiente' ? 'La fecha y el comprobante se registran cuando confirmes que el pago fue recibido.' : status === 'cancelado' ? 'El pago fue cancelado; no requiere fecha ni comprobante.' : 'Selecciona el estado del pago para continuar.'; }
...
function status(v) { return ({ planificado: 'Planificado', en_curso: 'En curso', finalizado: 'Finalizado', pausado: 'Pausado', cancelado: 'Cancelado' })[v] || v; }
function projectStatusClass(v) { return v === 'en_curso' ? 'progress' : v === 'cancelado' ? 'cancelled' : ''; }
function paymentStatus(v) { return ({ pendiente: 'Pendiente', confirmado: 'Confirmado', cancelado: 'Cancelado' })[v] || v; }
function paymentStatusClass(v) { return v === 'pendiente' ? 'progress' : v === 'cancelado' ? 'cancelled' : ''; }
